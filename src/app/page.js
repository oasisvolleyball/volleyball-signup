...
export default function App() {
  const [view, setView] = useState('signup');         // 'signup' | 'admin' | 'list'
  const [sessions, setSessions] = useState([]);        // all open sessions from sheet
  const [players, setPlayers] = useState([]);          // player directory from sheet
  const [selectedSession, setSelectedSession] = useState(null); // session player picked
  const [listSession, setListSession] = useState(null);         // session host is viewing in list
  const [signups, setSignups] = useState([]);          // signups for selectedSession or listSession
  const [draft, setDraft] = useState({ ...EMPTY_SESSION, id: genId() }); // admin form state
  const [editingId, setEditingId] = useState(null);   // which session is being edited
  ...
  // Load players + sessions on mount
  useEffect(() => {
    fetch('/api/signup')
      .then(r => r.json())
      .then(d => {
  ...
  // The active session for fetching signups is whichever is selected
  const activeDate = (view === 'list' ? listSession?.date : selectedSession?.date) || null;
  ...
  // Spots calculation for selected session
  ...
  // Spots for each session card in the picker
  function spotsLeft(s) {
    // We don't pre-load signups for all sessions, show maxGames as a guide
    return s.maxGames;
  }

  // Type options for selected session — uses session prices, never hardcoded
  ...
  // Amount for this signup — always from session prices
  ...
  const removeSignup = async (name) => {
    const activeSession = view === 'list' ? listSession : selectedSession;
    if (!activeSession) return;
    try {
      await fetch('/api/signup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'remove_signup', date: activeSession.date, name }),
      });
      fetchSignups();
    } catch (e) {}
  };
  ...
  const S = `
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .nb { flex:1; min-width:72px; padding:12px 6px; text-align:center; border:none; background:none; font-size:12px; font-weight:600; color:#64748b; cursor:pointer; border-bottom:3px solid transparent; margin-bottom:-1px; transition:all 0.2s; white-space:nowrap; }
    ...
    .sess-card-title { font-family:'Syne',sans-serif; font-size:15px; font-weight:800; color:#f1f5f9; margin-bottom:4px; }
    ...
    .no-sess { font-size:13px; color:#475569; text-align:center; padding:20px; }
    .sw { max-width:480px; margin:0 auto; }
    .picker { padding:16px; }
    .picker-title { font-family:'Syne',sans-serif; font-size:11px; font-weight:800; letter-spacing:3px; text-transform:uppercase; color:#475569; margin-bottom:12px; }
    .pick-card { background:#1e293b; border:1.5px solid #334155; border-radius:12px; padding:14px 16px; margin-bottom:10px; cursor:pointer; transition:all 0.2s; position:relative; }
    .pick-card:hover { border-color:#475569; }
    .pick-card.sel { border-color:#f59e0b; }
    .pick-title { font-family:'Syne',sans-serif; font-size:15px; font-weight:800; color:#f1f5f9; }
    .pick-meta { font-size:12px; color:#64748b; margin-top:3px; }
    .pick-price { font-family:'Syne',sans-serif; font-size:16px; font-weight:800; color:#f59e0b; position:absolute; right:16px; top:14px; }
    ...
    .sc { background:linear-gradient(135deg,#1e293b,#0f172a); padding:24px 20px 20px; border-bottom:2px solid #f59e0b33; position:relative; overflow:hidden; }
    ...
    .sc-back { font-size:12px; color:#64748b; cursor:pointer; margin-bottom:10px; display:inline-flex; align-items:center; gap:4px; transition:color 0.2s; }
    ...
    .sc-icon { font-size:14px; width:18px; text-align:center; flex-shrink:0; margin-top:1px; }
    ...
    .sugg { padding:10px 14px; cursor:pointer; display:flex; justify-content:space-between; align-items:center; transition:background 0.15s; font-size:13px; color:#f1f5f9; font-weight:600; }
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", minHeight: '100vh', background: '#0f172a', color: '#f1f5f9' }}>
      ...
      <div className="nav">
        ...
        <button className={`nb${view === 'list' ? ' on' : ''}`} onClick={() => setView('list')}>
          📋 List {listSession && <span className="badge">{signups.length}</span>}
        </button>
      </div>
          ...
          <div className="aw">
            ...
            {sessions.length > 0 && (
              <>
                ...
                <div className="st" style={{ marginTop: 24 }}>{editingId ? 'Edit Session' : 'New Session'}</div>
              </>
            )}
            {sessions.length === 0 && <div className="st">Create Session</div>}
            ...
            <p style={{ textAlign: 'center', fontSize: 12, color: '#475569', marginTop: 10 }}>
              Share the Sign Up link in WhatsApp after publishing
            </p>
          </div>
        )
      )}
      ...
      {view === 'signup' && (
        sessions.length === 0 ? (
          ...
          // Signup form for selected session
          <div className="sw">
            <div className="sc">
              <div className="sc-back" onClick={() => { setSelectedSession(null); setSubmitted(false); }}>
                ← Back to sessions
              </div>
      ...
      {view === 'list' && (
        !adminUnlocked ? (
          <div className="lock-wrap">
            <div className="lock-title">🔒 Host Access</div>
            <input className="inp" type="password" placeholder="Password…"
              value={adminPassword} onChange={e => setAdminPassword(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && adminPassword === PASSWORD && setAdminUnlocked(true)} />
            <button className="pub-btn" style={{ marginTop: 14 }}
          ...
          // Session picker for list
          <div className="list-sess-pick">
            ...
            {sessions.length === 0 && <div className="no-sess">No open sessions.</div>}
            {sessions.map(s => (
              <button key={s.id} className={`list-sess-btn${listSession?.id === s.id ? ' sel' : ''}`}
                onClick={() => setListSession(s)}>
          ...
          <div className="lw">
            <div className="lh">
              <div>
                ...
                <div style={{ fontSize: 12, color: '#475569', marginTop: 2, cursor: 'pointer' }} onClick={() => setListSession(null)}>
                  ← Change session
                </div>
              </div>
              <button className="exp-btn" onClick={() => {
                const rows = ['Name,Type,Amount (AED),Paid',
                  ...signups.map(s => `${s.name},${s.type},${s.amount},${s.paid}`)];
                const blob = new Blob([rows.join('\n')], { type: 'text/csv' });
                const a = document.createElement('a');
                a.href = URL.createObjectURL(blob);
                a.download = `signups-${listSession.date}.csv`;
                a.click();
              }}>⬇ Export CSV</button>
            </div>
            ...
            {signups.length === 0 && (
              <div style={{ textAlign: 'center', color: '#334155', padding: '40px 0', fontSize: 14 }}>No signups yet.</div>
            )}
